...
class CrystalKingdoms:
    def __init__(self):
        ...
        self.quest_data = {
            ...
            "Reclaim the Corrupted Throne": {
                ...
                "enemy_kills": {"Dark Knight": 1, "Shadow Mage": 1},
    ...
    def show_status(self):
        ...
        print(f"Inventory: {', '.join(item.replace('_', ' ') for item in self.player.inventory) or 'Empty'}")
    ...
    def use_item(self):
        if self.player.inventory:
            self.handle_item_effect(self.choose_item())
    ...
    def choose_item(self):
        ...
        try:
            return self.player.inventory[int(input("\nChoose item to use (0 to cancel): ")) - 1]
    ...
    def use_item(self):
        if self.player.inventory:
            self.handle_item_effect(self.player.inventory[self.choose_item()])
    ...
    def handle_item_effect(self, item: str):
        ...
        if item_data["type"] == "consumable":
            ...
            self.player.strength += item_data["value"]
            print(f"\nStrength increased by {item_data['value']}!")
        self.player.inventory.pop(self.choose_item())
    ...
    def check_game_over(self) -> bool:
        ...
        enough_choices = len(self.player.story_choices) >= 5
        
        if all_quests_completed or enough_choices:
            self.trigger_ending()
            return True
        return False

    def trigger_ending(self):
        crystal_visits = self.player.story_choices.count("crystal_cave")
        castle_visits = self.player.story_choices.count("corrupted_castle")
        forest_visits = self.player.story_choices.count("haunted_forest")
        if crystal_visits > castle_visits and crystal_visits > forest_visits:
            print("\n=== The Crystal Guardian Ending ===")
        elif castle_visits > crystal_visits and castle_visits > forest_visits:
            print("\n=== The Shadow Emperor Ending ===")
        else:
            print("\n=== The Forest Sage Ending ===")
